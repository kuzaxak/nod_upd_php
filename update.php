...
foreach ($servers as $server)
{
    ...
    if($server['ess']==false){
        foreach (${'section_no_ess_'.$server['type']} as $not_exist_module => $val11) {
            unset($section1[$not_exist_module]);
        }
    }
    ...
    foreach ($upd_ser2 as $upd_ser3)
    {
        ...
        foreach ($files1 as $file)
        {
            ...
            if (isset($file_nup[1])==true && $file_nup[1]=="nup")
            {
                ...
                $del_file=0;
                if (isset($data['UPDATE_INFO']['name']) && isset($updatedb[$data['UPDATE_INFO']['name']]) && (isset($section2[$data['UPDATE_INFO']['name']]) || isset($section1[$data['UPDATE_INFO']['name']])))
                {
                ...
                if($del_file==false)$new_update[$data['UPDATE_INFO']['name']]['size']=filesize($savepath.$name_upd);
